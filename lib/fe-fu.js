...
function toJS(path){
	fs.readFile(path, 'utf-8', function (e, str) {
		...
		try {
			if (defaults.jsLint && jsLintContent(str))
				return;
...
function jsLintContent(str){
	var isOk = jsLint.check(str);//.replace(/^\#\!.*/, ''));
	if (isOk){
		return false;
	}
    ...
    message.push('JSLint found ' + errors.length + ' errors\n');
	message.push((isCritical ? '' : 'Non') + ' critial error..\n\n');
	for (var err in errors){
		var curr = errors[err];
		message.push(err + ': Line (' + curr.line + ') Char (' + curr.character + ')\n');
		message.push('Reason : ' + curr.reason + '  Evidence : ' + curr.evidence + '\nRaw: ' + curr.raw + '\n');
		var details = [curr.a, curr.b, curr.c, curr.d];
		for (var det in details){
			if (details[det]){
				message.push('Detail ' + det + ': ' + details[det] + '\n');
			}
		}
		message.push('\n');
	}
	Notify(message.join(''), isCritical);
	errors = null;
	return true;
}
